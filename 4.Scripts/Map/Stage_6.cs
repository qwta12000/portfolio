...
public class Stage_6 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MgerClass.InstFunc.chek_boss = true;
        
        MgerClass.InstFunc.sevestage("Stage_6");
            ...
            for (int z = 0; z < znode; z++)
            {
                ...
                else
                {
                    ...
                    else if (randNum == 4)
                    {
                        ...
                        {
                            Instantiate(kinetic_2, new Vector3(xpos, transform.position.y + 1.5f, zpos), Quaternion.identity);
    ...
    IEnumerator blink_eft()
    {
        while (count <= 8)
        {
            ...
            text_stage_6.text = "Stage 6";
            yield return new WaitForSeconds(0.1f);
            count++;

            if (count == 8)
            {
                text_stage_6.enabled = false;
